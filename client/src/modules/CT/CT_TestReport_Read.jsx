...
export default function CT_TestReport_Read() {
  ...
  const initialColumns = [
    ...
    { "key" : "kor", "label" : "국문", "sortable" : true },
    { "key" : "is_cpnp", "label" : "CPNP", "sortable" : true },
    { "key" : "is_eng", "label" : "영문(엑셀)", "sortable" : true },
    { "key" : "daily_judgment", "label" : "당일판정", "sortable" : true },
    { "key" : "final_judgment", "label" : "최종판정", "sortable" : true },
    { "key" : "officeDocument", "label" : "공문", "sortable" : true },
    { "key" : "remark", "label" : "기타", "sortable" : true }
  ];
  ...
  const [searchForm, setSearchForm] = useState({
    ...
    "date_from" : G_STARTDAY,   // 기본값은 오늘로부터 7일 전 날짜
    "date_to" : G_TODAY,        // 기본값은 오늘 날짜
    "ct_no" : "",               // CT 번호 검색어
   ...
   * ----------------------------------------------------
   * HOW : useMemo를 사용하여 reports, 검색어(debouncedQuery), 날짜범위(from/to),
   *       정렬 기준(sortKey, sortDir)이 변경될 때만 재계산한다.
   *       1) 검색어 전처리(소문자 + trim)
   *       2) reports 리스트에서 검색어, 카테고리, 기간 조건을 모두 충족하는 데이터만 필터링
  ...
  }, [reports, debouncedQuery, type, from, to, sortKey, sortDir]);
   ...
   * 페이지 로딩 시 초기 검색 실행
   * ----------------------------------------------------
   * HOW : useEffect로 초기 검색 함수 호출
   * WHY : 페이지 진입 시 기본 데이터가 표시되어 사용성 향상
   */
  useEffect(() => {
    initialSearch();
  }, []);
        ...
        <div>
          ...
          <div className="search-information">
            <form className="search-form">
              ...
              {/* ↓ 시험 성적서 조회 조건값 ↓ */}
              <div className="test-report-search-grid test-report-search-grid-line-1">
                <div className="test-report-search-grid-label">조회조건</div>
                <div className="test-report-search-grid-inline">
                ...
                <div className="test-report-search-grid-label">CT 번호</div>
                <div className="test-report-search-grid-inline">
                ...
                <div className="test-report-search-grid-label">검색어</div>
                <div className="test-report-search-grid-inline">
                  ...
                  <tr key={idx}>
                    {columns.map((col) => (
                      <td key={`${idx}-${col.key}`} data-label={col.label}>
                        <Link 
                          to={`/ct/testReport/report?ct_request_id=${item.ct_request_id}&ct_test_report_id=${item.ct_test_report_id}&mode=update`}>
