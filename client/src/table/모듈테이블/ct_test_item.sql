-- CT 시험 항목 테이블 (시험 성적 상세)
CREATE TABLE `ct_test_item` (
  `ct_test_item_id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'PK',
  `company_id` int(11) NOT NULL DEFAULT 1 COMMENT '회사 pk(외래키)',
  `ct_test_report_id` int(11) NOT NULL COMMENT 'CT 시험 성적서 pk(외래키)',
  `test_id` varchar(50) DEFAULT NULL COMMENT '시험기준 pk(외래키)',
  `test_standard` text DEFAULT NULL COMMENT '시험 기준',
  `test_result` text DEFAULT NULL COMMENT '시험 성적',
  `remark` text DEFAULT NULL COMMENT '종합의견',
  `note` text DEFAULT NULL COMMENT '비고',
  `attached_image_url` varchar(500) DEFAULT NULL COMMENT '첨부 이미지 URL',
  ...
  PRIMARY KEY (`ct_test_item_id`),
  KEY `idx_ct_test_report_id` (`ct_test_report_id`) COMMENT '성적서 ID별 조회',
  KEY `idx_test_id` (`test_id`) COMMENT '시험기준 번호별 조회',
  KEY `idx_company_active_sort` (`company_id`, `is_active`, `sort_order`) COMMENT '회사, 활성 여부 및 정렬 순번 인덱스'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='CT 시험 항목 테이블';