 ...
 * @param n : 추가할 일 수
 */
export const addDayCalendar = (dateStr, n) => {
    const d = new Date(dateStr + "T00:00:00");
 ...
 * @returns 
 */
export const cleanParamsArray = (arr) => arr.map(cleanParams)
 ...
 * @returns 
 */
export const noticeValidation = (message) => (
    <>
 ...
 */
export const generateMockCTData = (count) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        data.push({
            id: `CT2025-${number}`,
            idx: randomPick(idxValues),
            kor: randomPick(korValues),
            cpnp: randomPick(cpnpValues),
            eng: randomPick(engValues),
            todayJudge: randomPick(todayJudgeValues),
            weekJudge: randomPick(weekJudgeValues),
            lastJudge: randomPick(lastJudgeValues),
            officeDocument: randomPick(officeDocumentValues),
            remark: randomPick(remarkValues),
 ...
 */
export const generateMockCTApproveData = (count) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        title: `CT2025-${number}`,
        labNo: `LAB-${number}`,
        type: randomPick(typeValues),
        name: `제품${i + 1}`,
        compName: randomPick(compNames),
        mateName: randomPick(mateNames),
        mateCompName: randomPick(mateCompNames),
    
        saleReqDate: randomDate(1 + (i % 10)),
        sampleReqDate: randomDate(1 + (i % 10)),
        ctReqDate: randomDate(1 + (i % 10)),
        ctRecptDate: randomDate(1 + (i % 10)),
        ctTestDate: randomDate(1 + (i % 10)),
    
        ctStatus: randomPick(ctStatusValues),
        judgement: randomPick(judgementValues),
    
        salesManager: randomPick(managerNames),
        sampleManager: randomPick(managerNames),
        ctManager: randomPick(managerNames),
    
        emerStatus: randomPick(statusValues.emer),
        cpnpStatus: randomPick(statusValues.cpnp),
        engStatus: randomPick(statusValues.eng),
    
        category: randomPick(categoryValues),
 ...
 * @returns 
 */
export const findMatchRanges = (text = "", keyword = "") => {

    if (!text || !keyword) return []; // keyword가 비어 있으면 매칭 범위가 없으므로 즉시 빈 배열 반환

    const source = normalize(text); // text와 keyword를 동일한 규칙으로 정규화
    const key = normalize(keyword); // 초성/영문 소문자/숫자 기반으로 비교 가능하게 통일
...
export const highlight = (text = "", keyword = "") => {
    ...
    const ranges = findMatchRanges(targetText, targetKeyword);
